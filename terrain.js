...
// Carrega uma imagem que representa o mapa de altura do terreno
const img = new Image();
img.onload = run;
img.crossOrigin = 'anonymous';
img.src = './terrain/heightmap.png';


function run() {
    // Cria um contexto 2D do canvas para ler a imagem
    ...
    // Função para obter a altura do terreno a partir dos dados da imagem
    ...
    // Inicialização das listas de posições, coordenadas de textura e índices
    const positions = [];
    const texcoords = [];
    const indices = [];
    ...
    for (let z = 0; z < cellsDeep; ++z) {
        for (let x = 0; x < cellsAcross; ++x) {
            // Cálculo das alturas dos vértices
            ...
            // Cálculo dos índices dos vértices para criar os triângulos
            indices.push(
                ndx, ndx + 4, ndx + 1,
                ndx, ndx + 2, ndx + 4,
                ndx + 2, ndx + 3, ndx + 4,
    ...
    // Configuração do WebGL e carregamento de shaders
    ...
    void main() {
        // Deve fazer disso uma uniform para que seja compartilhado
        ...
        vec3 color = mix(vec3(0.0, 0.0, 0), vec3(1, 1, 1), groundTexture);
